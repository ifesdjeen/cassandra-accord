...
public abstract class Persist extends AbstractSimpleCoordination implements Callback<ApplyReply>
{
    ...
    public void onSuccess(Id from, ApplyReply reply)
    {
        ...
        switch (reply)
        {
            ...
            case Redundant:
            case Applied:
                if (tracker.recordSuccess(from) == Success && informDurableOnDone)
                {
    ...
    protected void start()
    {
        ...
        {
            super.start();
            node.send(contact, to -> factory.create(applyKind, to, tracker.topologies(), txnId, ballot, sendTo, txn, executeAt, stableDeps, writes, result, route, flags.get(to)), executor, this);
        }
    }
