...
abstract class Propose<R> extends AbstractCoordination<R, AcceptReply>
{
    ...
    void start()
    {
        ...
        {
            super.start();
            node.send(contact, to -> new Accept(to, tracker.topologies(), kind, ballot, txnId, route, executeAt, deps, require != route), executor, this);
        }
    }
    ...
    static class NotAccept extends AbstractCoordination<Void, AcceptReply> implements Callback<AcceptReply>
    {
        final Status status;
        final Ballot ballot;
        final TxnId txnId;
        ...
        {
            super.start();
            node.send(tracker.nodes(), to -> new Accept.NotAccept(status, ballot, txnId, someParticipants), executor, this);
        }
